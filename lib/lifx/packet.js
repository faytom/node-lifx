...
Packet.headerToObject = function(buf) {
  ...
  // Frame
  var bu = new Buffer(buf);
  obj.size = bu.readUInt16LE(offset);
  offset += 2;
  var frameDescription = bu.readUInt16LE(offset);
  ...
  obj.source = bu.toString('hex', offset, offset + 4);
  offset += 4;

  // Frame address
  obj.target = bu.toString('hex', offset, offset + 6);
  offset += 6;

  obj.reserved1 = bu.slice(offset, offset + 2);
  offset += 2;

  obj.site = bu.toString('utf8', offset, offset + 6);
  obj.site = obj.site.replace(/\0/g, '');
  offset += 6;

  var frameAddressDescription = bu.readUInt8(offset);
  ...
  obj.sequence = bu.readUInt8(offset);
  offset += 1;

  // Protocol header
  obj.time = utils.readUInt64LE(bu, offset);
  offset += 8;

  obj.type = bu.readUInt16LE(offset);
  offset += 2;

  obj.reserved2 = bu.slice(offset, offset + 2);
  offset += 2;
  ...
  if (obj.type !== undefined) {
    ...
    if (packets[typeName] !== undefined) {
      if (typeof packets[typeName].toObject === 'function') {
        var bu = new Buffer(buf);
        var specificObj = packets[typeName].toObject(bu.slice(constants.PACKET_HEADER_SIZE));
