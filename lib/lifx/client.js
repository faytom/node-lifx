...
Client.prototype.init = function(options, callback) {
  ...
  chrome.sockets.udp.create(function(createInfo) {
    ...
    chrome.sockets.udp.onReceive.addListener(function(data) {
      // Ignore own messages and false formats is not ran here
      // console.log("onReceive Data, ", data)
      // var view = new Uint8Array(data.data);
      // console.log('onReceive, ', view)
      if(data.data.byteLength == 36){
        console.log('self return message')
        return;
      }
      // console.log('onReceive, ', data.data.toString('utf8'));
      // console.log(JSON.stringify(data.data))
      console.log('addListener启动')
      // Parse packet to object
      var parsedMsg = Packet.toObject(data.data);
      ...
      if (parsedMsg instanceof Error) {
        ...
        // console.error('Packet: ', data.msg.toString('hex'));
        ...
        self.processMessageHandlers(parsedMsg, data);

        self.emit('message', parsedMsg, data);
      }
    });

    // Socket binding process
    chrome.sockets.udp.bind(self.socket, opts.address, opts.port, function() {
      console.log('bind启动')
      self.isSocketBound = true;
      // chrome.sockets.udp.setBroadcast(createInfo.socketId, true);
      chrome.sockets.udp.setBroadcast(self.socket, true, function(result){
        console.log('setBroadcast启动')
      });
      ...
      if (opts.startDiscovery) {
        self.startDiscovery(opts.lights);
      }
...
Client.prototype.sendingProcess = function() {
  ...
  if (this.messagesQueue.length > 0) {
    ...
    if (msg.transactionType === constants.PACKET_TRANSACTION_TYPES.ONE_WAY) {
      ...
      var buf = this.toArrayBuffer(msg.data);
      // console.log('arrayBuffer length ', buf.byteLength)
      // chrome.sockets.udp.send(this.socket, msg.data, 0, msg.data.length, this.port, msg.address);
      chrome.sockets.udp.send(this.socket, this.toArrayBuffer(msg.data), msg.address, this.port, function(result){
...
// Client.prototype.toArrayBuffer = function(arr){
//   return new Uint8Array(arr).buffer;
// }

Client.prototype.toArrayBuffer = function(buf) {
...
Client.prototype.processMessageHandlers = function(msg, rinfo) {
  ...
  console.log('processMessageHandlers')
