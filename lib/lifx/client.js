...
function Client() {
  ...
  this.udpOnReceiveCallbackRegistered = false;
}
...
Client.prototype.init = function(options, callback) {
  ...
  if (!self.udpOnReceiveCallbackRegistered){
    self.udpOnReceiveCallbackRegistered = true;
    ...
    chrome.sockets.udp.onReceive.addListener(function(data) {
      ...
      if (!(data.data instanceof ArrayBuffer && data.data.byteLength !== undefined)){
        // console.log('error return message')
        return;
      }
      ...
      if (parsedMsg.target == '000000000000'){
